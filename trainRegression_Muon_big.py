...
python3 trainRegression_Muon_big.py 2>&1 | tee traininglog_regressionMuon_big.txt
"""
...
if any(log10maskWithParameters): model.add_module('log10Transform', LogTransform(mask=log10maskWithParameters, base=10))
...
if any(log10maskWithoutParameters): model.add_module('Log10TransformBack', LogTransformBack(mask=log10maskWithoutParameters))
...
for epoch in range(num_epochs):
    ...
    if epoch == 0:
        ...
        for data_loader, benchmarks, start_points in zip([train_loader, validation_loader], [benchmarks_train, benchmarks_validation], [start_points_train, start_points_validation]):
            for batch, (inp, target, _) in enumerate(data_loader):
                ...
                out = model(inp)
                if torch.isnan(inp).any() or torch.isnan(out).any() or torch.isnan(target).any():
                    print(f"NaN detected in inputs/outputs/targets before loss computation at epoch {epoch}, batch {batch}")                
    ...
    for batch, (inp, target, _) in enumerate(train_loader):
        ...
        for loss in loss_fns:
            loss_val = loss_fns[loss](inp, out, target)
            if torch.isnan(loss_val):
                print(f"NaN detected in {loss} loss at epoch {epoch}, batch {batch}")
        ...
        for name, param in model.named_parameters():
            if param.grad is not None and torch.isnan(param.grad).any():
                print(f"NaN gradient detected in {name} at epoch {epoch}, batch {batch}")
        trainer.step()
        trainer.zero_grad()

        epoch_train_loss += train_loss.item()
        if math.isnan(epoch_train_loss):
            print(f"NaN detected in epoch train loss at the end of epoch {epoch}")
    ...
    with torch.no_grad():
        ...
        for batch, (inp, target, _) in enumerate(validation_loader):
            ...
            out = model(inp)
            if math.isnan(epoch_validation_loss):
                print(f"NaN detected in inputs/outputs/targets before loss computation at epoch {epoch}, batch {batch}")                            
            ...
            if math.isnan(epoch_validation_loss):
                print(f"NaN detected in epoch validation loss at the end of epoch {epoch}")            
            if epoch + 1 == num_epochs:
                for loss in loss_fns:
